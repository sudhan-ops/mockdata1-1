...
const MapView: React.FC<{ events: (AttendanceEvent & { userName: string })[], users: User[] }> = ({ events, users }) => {
    ...
    useEffect(() => {
        if (mapContainerRef.current && !mapRef.current) {
            mapRef.current = L.map(mapContainerRef.current).setView([12.9716, 77.5946], 12); // Bangalore
        ...
        // Invalidate size to fix rendering issues in tabs/conditional components
    ...
    useEffect(() => {
        ...
        // FIX: Explicitly type the Map to avoid any.
        const latestUserLocations = new Map<string, AttendanceEvent & { userName: string }>();
        events.forEach(event => {
            if (event.latitude && event.longitude) {
                // FIX: Ensure get on a potentially empty map doesn't throw a runtime error.
        ...
        // FIX: Explicitly type the Map to ensure correct type inference for its values, resolving the error on `userMap.get`.
        const userMap = new Map<string, User>(users.map(u => [u.id, u]));
        const markerInstances: L.Marker[] = [];

        // FIX: Explicitly type the 'event' parameter to help with type inference.
        latestUserLocations.forEach((event: AttendanceEvent & { userName: string }) => {
            const user = userMap.get(event.userId);
            // FIX: Removed duplicated and nested if block which caused a syntax error.
