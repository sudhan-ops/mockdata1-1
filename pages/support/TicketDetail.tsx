...
const TicketDetail: React.FC = () => {
    ...
    const handleTicketUpdate = async (updates: Partial<SupportTicket>) => {
        ...
        try {
            ...
            const updatedTicket = await api.updateSupportTicket(ticket.id, updates, user.id, user.role);
            setTicket(updatedTicket);
            if(updates.status) setToast({ message: `Post status updated to ${updates.status}`, type: 'success' });
        } catch (e: any) {
            setToast({ message: e.message || 'Failed to update post.', type: 'error' });
        }
    };
    ...
    const renderActionButtons = () => {
        if (!user || ticket.status === 'Closed') return null;

        const isAdmin = user.role === 'admin';
        const isAssignedTo = user.id === ticket.assignedToId;

        return (
            <div className="flex flex-wrap gap-2">
                {ticket.status === 'Open' && (isAdmin || isRequester) && (
                    <Button
                 ...
                 {ticket.status === 'In Progress' && (isAdmin || isAssignedTo) && (
                    <Button
                        onClick={() => handleTicketUpdate({ status: 'Resolved', resolvedAt: new Date().toISOString() })}
                        disabled={!isAdmin && !isAssignedTo}
                    >
                        Mark as Resolved
                    </Button>
                )}
                 {ticket.status === 'Resolved' && (isAdmin || isRequester) && (
                    <Button
