...
const CameraCaptureModal: React.FC<CameraCaptureModalProps> = ({ isOpen, onClose, onCapture, captureGuidance = 'none' }) => {
  ...
  const startCamera = useCallback(async (deviceId?: string) => {
    ...
    try {
      ...
      if (videoTrack) {
        ...
        // Cast `capabilities` to `any` to access the `torch` property, which may not be in the default TypeScript lib definition.
  ...
  useEffect(() => {
    const initialize = async () => {
        if (isOpen) {
            ...
            try {
                ...
                // This is the crucial fix: stop the temporary permission-priming stream.
                if (stream) {
    ...
    initialize();
    return () => stopCamera();
  }, [isOpen, stopCamera, captureGuidance]);
